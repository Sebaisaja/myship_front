...
const UserRpTable = ({ rapidposts, setRefresh }) => {
  const [show, setShow] = useState(false);

  const deleteHandler = (id) => {
    if (window.confirm('êtes-vous sûr de vouloir supprimer')) {
      authAxios
        .delete(`/rapidPosts/${id}`)
        .then((res) => {
                  ...
                  <Link
                    to={
                      courrier?.coli?.country
                        ? `/rapidPosts/international-print/${courrier?._id}`
                        : `/rapidPosts/national-print/${courrier?._id}`
                    }
                  >
                    <FaPrint size={'1.3rem'} />
                  </Link>
